...
func (r *renderer) renderBlocks(blocks []ast.Block) error {
	for _, b := range blocks {
		if err := r.renderBlock(b); err != nil {
			return err
		}
	}
	return nil
...
func (r *renderer) renderBlock(block ast.Block) error {
	switch b := block.(type) {
	case *ast.TextBlock:
	...
	case *ast.DisplayBlock:
		displayValue, err := r.evalExpr(b.Value)
		if err != nil {
			return err
		}
	...
	case *ast.IfBlock:
		...
		if b.Alternative != nil {
			if err := r.renderBlocks(*b.Alternative); err != nil {
				return err
			}
		}
		return nil
...
func (r renderer) evalCall(function ast.Identifier, argumentValues []value.Value) (value.Value, error) {
	fn, err := builtin.LookupFunction(function)
	if err != nil {
		return nil, err
	}

	if len(fn.ArgTypes) != len(argumentValues) {
		return nil, fmt.Errorf("function %s expects %d arguments, got %d", fn.Name, len(fn.ArgTypes), len(argumentValues))
	}

	res, err := fn.Call(argumentValues)
	if err != nil {
		return nil, err
	}
	return res, nil
